...
export class Vessel {
    ...
    constructor(Name : Number, path : path){
        ...
        let material = new THREE.MeshLambertMaterial({ color: 0x42f4f4 }); //'#'+(Math.random()*0xFFFFFF<<0).toString(16) });
    ...
    public move(trafficLightArr : TrafficLight[], carArr: Vessel[]){
        ...
        for(let i = 0; i < this._pathLight.length; i++){
            ...
            trafficLightArr.forEach(light => {
                if(light.getGroupId == this._pathLight[i][0] && light.getId == this._pathLight[i][1] && light.getGroup == "vessel"){
                    trafficLight = light;
                }
            });
        ...
        carArr.forEach(vessel => {
            let carDistance = carFront.distanceTo(vessel._mesh.position);
            if(carDistance < 0.06){
                move = false;
            }
        });
